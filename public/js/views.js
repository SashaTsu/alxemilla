...
<div class="lang-switcher">
    ...
    <!-- 2. Меню, будет скроллиться отдельно -->
...
></a>`,
  ...
  if (Array.isArray(p.sizes) && p.sizes.length > 0) {
    selectedSize =
      p.sizes.find((size) => p.stock && Number(p.stock[size]) > 0) ||
      p.sizes[0];
  }
  let sizeBlock = "";
 ...
 <div class="product-info-block">
  ...
  function openProductTabModal(selectedKey = "materials") {
    ...
    if (!modal) {
      ...
      modal.querySelector(".product-tab-modal-close").onclick =
        modal.querySelector(".product-tab-modal-bg").onclick = () =>
          modal.remove();
    }
  ...
  if (sizeSelect) {
    ...
    function updateStock() {
      ...
      const stock =
        p.stock && typeof p.stock[val] !== "undefined"
          ? Number(p.stock[val])
          : null;
      let msg = "";
  ...
  if (downloadBtn && p.fileUrl) {
    ...
    downloadBtn.addEventListener("click", function (e) {
      e.preventDefault();
      const now = Date.now();
      if (now - lastClick < wait || downloadBtn.classList.contains("loading"))
        return;
      lastClick = now;
...
`;
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (preColor) {
      payload.color =
        typeof preColor === "string" ? preColor : getColorTitle(preColor);
    }
