...
export async function renderAdminForm(id = null) {
  ...
  categoryInput.addEventListener("input", function () {
    if (categoryInput.value.trim() === "pattern")
  ...
  renderExtraImages();
  document.getElementById("add-extra-image-btn").onclick = () =>
    addExtraImage();
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!imageValid(mainImgObj))
      return alert("Основное фото — загрузите все 3 формата");
  ...
  function bindImageUploadBlock(prefix, imgObj, onChange) {
    ...
    document.getElementById(`${prefix}-file-jpeg-input`).onchange = async (
      e,
    ) => {
    ...
    document.getElementById(`${prefix}-file-webp-input`).onchange = async (
      e,
    ) => {
      const file = e.target.files[0];
      if (!file) return;
      ...
      webpStatus.textContent = imgObj.webpUrl
        ? "WebP загружен"
        : "WebP не загружен";
      clearBtn.style.display = "";
      onChange({ ...imgObj });
    ...
    document.getElementById(`${prefix}-file-avif-input`).onchange = async (
      e,
    ) => {
      const file = e.target.files[0];
      if (!file) return;
      ...
      avifStatus.textContent = imgObj.avifUrl
        ? "AVIF загружен"
        : "AVIF не загружен";
      clearBtn.style.display = "";
      onChange({ ...imgObj });
